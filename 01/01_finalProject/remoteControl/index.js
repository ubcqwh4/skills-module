...
const socket = io('http://10.97.165.188:9876') //find your IP in the terminal when you first connect in terminal
...
socket.on('touch', function(objTouch){
  ...
  var range = 15
  mouseX = map(objTouch.touchX,0,1,-range,range)
  mouseY = map(objTouch.touchY,0,1,range,-range)
});
////////////
...
function render () {
  ...
  //sending event/data from remote to server
  socket.emit('test',objToSend)
